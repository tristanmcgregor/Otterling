...
Dashboard-configured website rules (habit-gated schedule windows, or an exceeded
dailyBudgetMinutes budget) are deliberately NOT enforced here anymore: this mux used to NXDOMAIN a
habit-gated domain (e.g. youtube.com) directly, but an NXDOMAIN never lets a browser get far
enough to make an HTTP request at all, so it could only ever show a bare browser DNS-error screen,
never a real "you're blocked" page. Those domains now resolve normally and fall through to the
checks below like any other domain -- the actual enforcement (and the real /blocked page) now
happens in mitm_nsfw_addon.py, which essentially all of this household's 80/443 traffic already
tunnels through via CONNECT using a CA every managed device already trusts (see that file's
_WebsiteRuleBlocks/_BudgetTargets/_UsageTracker). The accepted tradeoff: non-web-port traffic to a
rule-gated domain is no longer blocked at the DNS layer, only its 80/443 web traffic.
